...
Note that my intention is not to give formal definitions (those are available in excess elsewhere on the internet) but rather an intuitive sense of the term in the context of my writings.

```{glossary}
Computer
  A dynamical system coupled with its environment. Colloquially, the 'computer' is the physical system that realizes a computation and the environment is a human distinguishing states or interpreting the result of a computation.

Programming
  The method by which a computer is configured. This configuration can be done via textual interface and include the set of algorithms that are conditionally and iteratively controlled (ie Python code on your laptop) or it can be encoding an initial state that evolves toward an attractor or some other way.

Computation
  A physical process that is also an information process.

Information Media
  A physical system capable of carrying information.
...
[Information Media](https://www.constructortheory.org/wp-content/uploads/2016/03/ct-info.pdf)
  Any physical medium/system whose state/configuration has the following properties: 1) can be copied, and 2) can be something else (ie. can be fliped). This forms the basis of a universal set in information theory. The fact that the receiver of said information can distinguish between at least two states in a medium and that the state could be at least one other thing makes it possible for that system to carry information and thus be an information medium.

P-system
  (In the context of membrane computing) A cell-like membrane structure which consists of regions (compartments) that are separated by membranes from each other and are organized in a tree structure. Each region (which may be empty or have objects in it) has a set of rules. In P-systems theory, networks of regions are also considered.
...
STP
  Standard temperature and pressure. Like beach weather 🏖️

Embodied Computation
  Computation in which the physical realization of the computation of the physical effects of the computation are essential to the computation.

Space
  That is--the mathematical kind. Imagine you have a bag of various balls—like soccer balls, basketballs, and ping-pong balls—and you want to talk about how to measure their size, how to add two balls together to get a new kind of ball, and how to make a ball bigger or smaller (scaling). In math, a "space" is like a bag—it's a collection of things you're interested in. The objects and the types of things you how you want to combine the things in the bag create the structure of the space.

Structure
  Generally, when I talk about the structure of a space I am referring to the rules that govern how you can combine objects in the space.

Linear Space
  Linear spaces mean that we can talk about addition and scalar multiplication of objects in the space.
...
Tensor product
  A particular way to build a new vector space from old vector spaces

Limits and colimits
  A way to construct new mathematical objects from old ones
...
Phase Space
  A space that shows all possible positions and momenta of the particles in the system. A complete map of where everything is and where it could possibly go.

Bijection
  A one-to-one mapping between two sets of objects.

Monad
  A structure that represents computations defined as sequences of steps. (Can put things in parallel)
